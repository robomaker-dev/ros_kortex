...
bool RobotiqGripperCommandActionServer::is_goal_tolerance_respected()
{
    ...
    return  m_goal.getGoal()->command.position < m_math_util.absolute_position_from_relative(actual_gripper_position + ROBOTIQ_GRIPPER_RELATIVE_ERROR, m_gripper_joint_limit_min, m_gripper_joint_limit_max) &&
            m_goal.getGoal()->command.position > m_math_util.absolute_position_from_relative(actual_gripper_position - ROBOTIQ_GRIPPER_RELATIVE_ERROR, m_gripper_joint_limit_min, m_gripper_joint_limit_max);
}
